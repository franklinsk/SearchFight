    ...
    public class Results
    {
        ...
        private Results(IReadOnlyList<string> languages, IReadOnlyList<string> runners, long[,] counts)
        {
            ...
            if (runners.Count != counts.GetLength(1))
                throw new InvalidOperationException("Counts second length must equal runners length.");
        ...
        public static async Task<Results> Collect(IReadOnlyList<string> languages, IReadOnlyList<ISearchRunner> runners)
        {
            if (languages == null)
                throw new ArgumentNullException("languages");
            ...
            for (var li = 0; li < languages.Count; li++)
            {
                for (var ri = 0; ri < runners.Count; ri++)
                {
                    tasks.Add(StartTask(languages, runners, results, li, ri));
                }
            }
        ...
        private static async Task StartTask(IReadOnlyList<string> languages, IReadOnlyList<ISearchRunner> runners, long[,] results, int li, int ri)
        {
